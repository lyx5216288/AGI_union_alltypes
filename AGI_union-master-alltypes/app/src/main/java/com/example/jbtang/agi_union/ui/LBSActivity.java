...
import com.example.jbtang.agi_union.dao.BaseStationinfoDBManager;
...
//    Marker[] markarr;
    ...
    public List<BaseStationinfo> bsinfolist;
    public BaseStationinfo bsinfo;
    private CellInfoDBManager cellInfoDBManager;
    private BaseStationinfoDBManager baseDBManager;

    private EditText tai,ecgi;
    private Button select;
    private Retrofit r;

    private SharedPreferences lbsSP;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        baseDBManager = new BaseStationinfoDBManager(getApplicationContext());
        bsinfolist=baseDBManager.listDB();
        for  ( int  i  =   0 ; i  <  bsinfolist.size()  -   1 ; i ++ )   {
            for  ( int  j  =  bsinfolist.size()  -   1 ; j  >  i; j -- )   {
                if  (bsinfolist.get(j).ecgi.equals(bsinfolist.get(i).ecgi))   {
                    bsinfolist.remove(j);
                }
            }
        }
        Log.e("LBSCreaterSize:",bsinfolist.size()+"");
        ...
        lbsSP=getSharedPreferences("lbsinfo",Context.MODE_PRIVATE);
        tai.setText(lbsSP.getString("tai",""));
        ecgi.setText(lbsSP.getString("ecgi",""));
        ...
        select.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(tai.getText().toString().length()>0&&ecgi.getText().toString().length()>0){
                    SharedPreferences.Editor editor=lbsSP.edit();
                    editor.putString("tai",tai.getText().toString());
                    editor.putString("ecgi",ecgi.getText().toString());
                    editor.commit();
                    boolean flag=true;
...
//        System.out.println(list);
        ...
        public void onReceiveLocation(BDLocation bdLocation) {
            ...
            GetMap service=r.create(GetMap.class);
                for(final CellInfoDAO cellinfo:cellInfoDAOList){
                    if (cellinfo.ecgi!=Integer.MAX_VALUE&&cellinfo.ecgi!=0){
                        ...
                        call.enqueue(new Callback<BaseStationinfo>() {
                            ...
                            public void onResponse(Call<BaseStationinfo> call, Response<BaseStationinfo> response) {
                                ...
                                if (bsinfo.getErrcode()==0){
                                    ...
                                    boolean flag1=true;
                                    for (BaseStationinfo info:bsinfolist){
                                        if (bsinfo.ecgi.equals(info.ecgi)){
                                            flag1=false;
                                        }
                                    }
                                    if (flag1){
            ...
            for(BaseStationinfo baseinfo:bsinfolist){
                            LatLng l=new LatLng(Double.parseDouble(baseinfo.getLat()),Double.parseDouble(baseinfo.getLon()));
                            CoordinateConverter converter= new CoordinateConverter();
                            converter.from(CoordinateConverter.CoordType.GPS);
                            // l待转换坐标
                            converter.coord(l);
    ...
    public void onDestroy() {
        ...
        baseDBManager.clear();
        baseDBManager.add(bsinfolist);
        Log.e("LBSDestroySize:",bsinfolist.size()+"");
