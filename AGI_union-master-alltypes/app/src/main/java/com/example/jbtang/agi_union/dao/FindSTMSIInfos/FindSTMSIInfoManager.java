...
public class FindSTMSIInfoManager {
    ...
    public FindSTMSIInfoManager(Context context) {
        ...
        db.execSQL("CREATE TABLE IF NOT EXISTS " + FindSTMSIInfoDBHelper.TABLE_NAME +
                "(stmsi TEXT PRIMARY KEY,pagingcount TEXT, count TEXT, time TEXT, pci TEXT, earfcn TEXT, ecgi TEXT, doubtful TEXT)");
    }
        ...
        try {
            for (FindSTMSI.CountSortedInfo countSortedInfo : cellInfos) {
                db.execSQL("INSERT INTO " + FindSTMSIInfoDBHelper.TABLE_NAME + " VALUES(?, ?,?, ?, ?, ?, ?, ?)",
                        new Object[]{countSortedInfo.stmsi,countSortedInfo.pagingcount, countSortedInfo.count, countSortedInfo.time, countSortedInfo.pci, countSortedInfo.earfcn, countSortedInfo.ecgi, countSortedInfo.doubtful?"true":"false"});
            }
    ...
    public void insert(FindeSTMSIInfoDAO dao) {
        db.execSQL("INSERT INTO " + FindSTMSIInfoDBHelper.TABLE_NAME + "VALUES(?,?,?,?,?,?,?,?)",
                new Object[]{dao.stmsi,dao.pagingcount, dao.count, dao.time, dao.pci, dao.earfcn, dao.ecgi,dao.doubtful});
    }
    ...
    public List<FindeSTMSIInfoDAO> listDB() {
        ...
        while (c.moveToNext()) {
            ...
            String pagingcount = c.getString(c.getColumnIndex("pagingcount"));
            ...
            FindeSTMSIInfoDAO findeSTMSIInfoDAO = new FindeSTMSIInfoDAO(stmsi,pagingcount, count, time, pci, earfcn, ecgi, doubtful);
