...
//            }
        ...
        if (currentActivity.getClass().equals(FindSTMSIActivity.class)){
            Global.pagingflag=true;
            Global.pagingindex++;
            FindSTMSI.initpaging();
        }
        send = false;
        deliver = false;
        ...
        if (serviceflag&&OrientationFindingActivity.linetype==1){
            if (OrientationFindingActivity.isquanxiang){
                try {
                    BluetoothService.getInstance().start(currentActivity,4);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }else {
                if (!OrientationFindingActivity.isdirect){
                    OrientationFindingActivity.shunxuindex++;
                }
                try {
                    BluetoothService.getInstance().start(currentActivity,OrientationFindingActivity.shunxuindex%4);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
        ...
        public void run() {
            ...
            if (send && !deliver) {
                ...
                currentActivity.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(currentActivity, "目标手机可能关机！", Toast.LENGTH_LONG).show();
                    }
                });
            }
        }
