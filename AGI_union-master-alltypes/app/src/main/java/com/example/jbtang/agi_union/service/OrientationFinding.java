...
public class OrientationFinding {
    ...
    private static final Integer MIN_RSRP = -125;
    private static final double SINR_THRESHOLD = -5D;
...
//    private Map<String,Timer> statusTimerMap;
        ...
        public boolean handleMessage(Message msg) {
            ...
            new Timer().schedule(new TimerTask() {
                ...
                public void run() {
                    ...
                    monitorDevice11.startMonitor(Status.Service.ORIENTATION);
                    monitorDevice11.setStartAgain(true);
                    if(cellTimerMap.get(monitorDevice11.getName())!=null){
                        cellTimerMap.get(monitorDevice11.getName()).cancel();
                        cellTimerMap.remove(monitorDevice11.getName());
                        cellTimerMap.put(monitorDevice11.getName(), new Timer());
                        cellTimerMap.get(monitorDevice11.getName()).schedule(new CellTimerTask(monitorDevice11.getName()), 10000);
    ...
    public static class OrientationInfo {
        ...
        private static final double MIN_RSRP = -125D;
...
//            currentActivity.findViewById(R.id.orientation_find_start).setEnabled(true);
            currentActivity.runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (OrientationFindingActivity.startToFind) {
                        OrientationFindingActivity.startToFind = false;
                        OrientationFindingActivity.stopButton.setEnabled(false);
                    }
                    stop();
...
//        }
        ...
        if (cellTimerMap.get(deviceName)!=null){
...
//            if (Global.Configuration.model == Status.Model.CARRY || (Global.Configuration.model == Status.Model.VEHICLE && deviceName.equals(DeviceManager.getInstance().getDevices().get(0).getName()))) {
                if (orientationType == R.id.orientation_find_trigger_single) {
                    if (monitorDevice.getUeCaptureTimes() == 0) {
                        ...
                        ueTimerMap.get(deviceName).schedule(new UeTimerTask(deviceName), Global.Configuration.silenceCheckTimer * 1000 / 4);
                        return;
                    }else if (monitorDevice.getUeCaptureTimes() == 2){
                        return;
                    }
                }
...
//        }
    ...
    class RsrpResult {
        ...
        private void add(UEInfo info, int type) {
            ...
            queue.add(info);
//            results.put(type,queue);//0613add gai
        }
